...
let timePressure = time |> Seq.map (fun x ->(x, ptaModel.PressureDrop(x, q)))
...
let chart =  Chart.Line (timePressure, Name ="pressure vs time", Color = Color.Blue)
            ...
            |> Chart.WithYAxis(Min = -2500.0, Log = false, LabelStyle = LabelStyle(Format = "F0"))
            |> Chart.WithYAxis(Enabled = true, Title = "pressure, [psi]") 
            

let data = [|(0.0,1000.0);(1000.0,0.0)|]

let timePressure2 = time |> Seq.map (fun x ->(x, ptaModel.PressureDrop(x, data)))
